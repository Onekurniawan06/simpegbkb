...
<?php
    ...
    if ($statusTerakhir === 'diproses') {
        ...
        // JALUR 1: Jika pengajuan Pensiun atau Pangkat
        if (in_array($sumber, ['pensiun', 'pangkat', 'pangkatgajitunjangan'])) {
            ...
            } elseif ($tahapSekarang === 'Direktur Kepatuhan') {
                $nextStepName = 'Direktur Utama';
            } elseif ($tahapSekarang === 'Direktur Utama') {
                $nextStepName = 'HRO';
            } else {
                $nextStepName = 'Tahap Selanjutnya';
            }
        }
        // JALUR 2: Jika pengajuan Lembur atau Cuti
        else {
            if ($tahapSekarang === 'Pengajuan Awal') {
                $nextStepName = 'Manager';
            } elseif ($tahapSekarang === 'Manager') {
                // Untuk Lembur lanjut ke SKKMR, untuk Cuti lanjut ke Dir Ops
                $nextStepName = ($sumber === 'lembur') ? 'Kepala SKK & SKKMR' : 'Direktur Operasional';
            } elseif (str_contains(strtolower($tahapSekarang), 'skk')) {
                $nextStepName = 'Direktur Operasional';
            } elseif ($tahapSekarang === 'Direktur Operasional') {
                $nextStepName = 'HRO';
            } else {
                $nextStepName = 'Tahap Selanjutnya';
            }
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 h-full divide-x divide-slate-100">
        ...
        <div class="lg:col-span-2 flex flex-col min-h-0 overflow-y-auto p-4 bg-slate-50/30">
            ...
            <div class="flex flex-wrap items-start gap-x-10 gap-y-4 mb-6">
                ...
                <div class="min-w-[120px]">
                    ...
                    <p class="text-sm text-slate-700 font-semibold"><?php echo e($data->jabatan); ?></p>
                </div>

                
                <?php if(in_array($sumber, ['pensiun', 'pangkatgajitunjangan', 'pangkat'])): ?>
                    <div class="min-w-[100px]">
                        <label class="block text-[10px] font-bold text-slate-400 uppercase mb-1">Masa Kerja</label>
                        <p class="text-sm text-slate-700 font-semibold"><?php echo e($data->masa_kerja ? preg_replace('/(?<=\d)(?=[a-z])|(?<=[a-z])(?=\d)/i', ' ', $data->masa_kerja) : '-'); ?></p>
                    </div>
                    <div class="min-w-[120px]">
                        ...
                        <p class="text-sm text-slate-700 font-semibold">
                            <?php echo e($data->tmt_pegawai ? \Carbon\Carbon::parse($data->tmt_pegawai)->translatedFormat('d F Y') : '-'); ?>
                                    ...
                                    <?php $__empty_1 = true; $__currentLoopData = $files ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $file): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                        <tr class="border-b border-slate-100 hover:bg-slate-50/50 transition">
                                            <td class="px-6 py-4 font-medium text-slate-700 whitespace-nowrap">
                                                <?php echo e(str_replace('_', ' ', $file->tipe_dokumen)); ?>

                                            </td>
                                            <td class="px-6 py-4 text-right">
                                                <button type="button"
                                                        
                                                        onclick="openPdfModal('<?php echo e(route('direktur.lihatDokumen', ['id' => $file->id])); ?>', '<?php echo e(str_replace('_', ' ', $file->tipe_dokumen)); ?>')"
                ...
                <?php elseif($sumber == 'lembur'): ?>
                    ...
                    <div class="ml-2 bg-blue-50/50 p-3 rounded-2xl border border-blue-100 space-y-5">
                        <div>
                            ...
                            <p class="text-sm text-slate-700 leading-relaxed italic">"<?php echo e($data->uraian_tugas ?? 'Tidak ada deskripsi'); ?>"</p>
                        </div>
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 pt-4 border-t border-blue-100/50">
                            <div class="flex flex-col">
                                ...
                                <span class="text-sm font-semibold text-slate-700 mt-1"><?php echo e(\Carbon\Carbon::parse($data->tanggal_lembur)->translatedFormat('d F Y')); ?></span>
                            </div>
                            <div class="flex flex-col">
                                <span class="text-[9px] font-bold text-slate-400 uppercase tracking-wider">Jam Mulai</span>
                                <span class="text-sm font-semibold text-emerald-600"><?php echo e($data->jam_mulai ? date('H:i', strtotime($data->jam_mulai)) : '-'); ?> WIB</span>
                            </div>
                            <div class="flex flex-col">
                                <span class="text-[9px] font-bold text-slate-400 uppercase tracking-wider">Jam Selesai</span>
                                <span class="text-sm font-semibold text-rose-600 mt-1"><?php echo e($data->jam_selesai ? date('H:i', strtotime($data->jam_selesai)) : '-'); ?> WIB</span>
                            </div>
                            <div class="flex flex-col">
                                <span class="text-[9px] font-bold text-blue-500 uppercase tracking-wider">Total Lembur</span>
                                <span class="text-sm font-bold text-blue-700 mt-1"><?php echo e($data->total_jam_lembur ?? '0'); ?></span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="ml-2 bg-indigo-50/50 p-4 rounded-2xl border border-indigo-100 space-y-5">
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-6 w-full">
                            ...
                            <div class="flex flex-col items-start">
                                ...
                                <span class="text-sm font-semibold text-slate-700 mt-1.5"><?php echo e(\Carbon\Carbon::parse($data->tanggal_mulai)->translatedFormat('d F Y')); ?></span>
                            </div>

                            
                            <div class="flex flex-col items-start">
                                <span class="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Tanggal Selesai</span>
                                <span class="text-sm font-semibold text-slate-700 mt-1.5"><?php echo e(\Carbon\Carbon::parse($data->tanggal_selesai)->translatedFormat('d F Y')); ?></span>
                            </div>

                            
                            <div class="flex flex-col items-start">
                                <span class="text-[9px] font-bold text-orange-500 uppercase tracking-widest">Lama Cuti</span>
                                <span class="text-sm font-bold text-orange-700 mt-1.5"><?php echo e($data->jumlah_cuti ?? 0); ?> <small class="font-normal">Hari</small></span>
                            </div>

                            
                            <div class="flex flex-col items-start">
                                <span class="text-[9px] font-bold text-emerald-500 uppercase tracking-widest">Sisa Cuti</span>
                                <span class="text-sm font-bold text-emerald-700 mt-1.5"><?php echo e($data->saldo_akhir ?? 0); ?> <small class="font-normal">Hari</small></span>
                            </div>
                        ...
                        <div class="border-t border-indigo-100/50 pt-4">
                            ...
                            <p class="text-sm text-slate-700 leading-relaxed italic">"<?php echo e($data->keterangan ?? 'Tidak ada keterangan'); ?>"</p>
                        </div>
                    </div>


                <?php elseif($sumber == 'pangkatgajitunjangan' || $sumber == 'pangkat'): ?>
        ...
        <div class="lg:col-span-1 flex flex-col bg-slate-50 overflow-hidden border-l border-slate-100 h-full">
            ...
            <div class="flex-1 overflow-y-auto custom-scroll-container p-3 pb-5">
                <form id="formApproval" action="<?php echo e(route('direktur.updateStatus', ['id_log' => $id_log, 'sumber' => $sumber])); ?>" method="POST" class="flex flex-col">
                    <?php echo csrf_field(); ?>
                        ...
                        <div class="bg-white p-3 rounded-xl border border-slate-200 shadow-sm space-y-3">
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div class="space-y-2">
                                    ...
                                    <input type="time" value="<?php echo e(isset($data->jam_mulai) ? date('H:i', strtotime($data->jam_mulai)) : '00:00'); ?>"
                                        readonly
                                ...
                                <div class="space-y-2">
                                    ...
                                    <input type="time" value="<?php echo e(isset($data->jam_selesai) ? date('H:i', strtotime($data->jam_selesai)) : '00:00'); ?>"
                                        readonly
                                        class="w-full p-3 text-sm border border-slate-100 rounded-xl bg-slate-50 font-medium text-slate-500 outline-none cursor-default">
                                </div>
                            </div>
                            ...
                            <div class="pt-2">
                                ...
                                <input type="text" value="<?php echo e($data->total_jam_lembur ?? '0 jam 0 menit'); ?>"
                                    readonly
                    ...
                    <div class="flex flex-col">
                        ...
                        <textarea name="catatan" rows="4"
                            ...
                            <?php if($data->status !== 'diproses'): ?> readonly <?php endif; ?>><?php echo e($data->komentar ?? ''); ?></textarea>
                    </div>
                    ...
                    <div class="grid grid-cols-1 gap-3 pt-2">
                        <button type="button" onclick="handleApproval('disetujui')"
                            <?php if($data->status !== 'diproses' && $data->status !== null): echo 'disabled'; endif; ?>
...
<?php echo $__env->make('layouts.app-direktur', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\simpegbkb\resources\views/direktur/detail_approval.blade.php ENDPATH**/ ?>