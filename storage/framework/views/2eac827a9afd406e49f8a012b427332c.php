        ...
        <div class="relative flex items-start">
            ...
            <?php $__currentLoopData = $historiLog; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $log): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                ...
                <div class="flex flex-col items-center flex-1 relative">
                    ...
                    <?php if(!$loop->last): ?>
                        <div class="absolute top-5 left-1/2 w-full h-[2px] z-0 <?php echo e($lineColor); ?>"></div>
                    <?php endif; ?>
                    ...
                    <div class="mt-4 text-center px-2">
                        <p class="text-[9px] font-bold text-gray-400 uppercase tracking-widest">Tahap <?php echo e($index + 1); ?></p>
                        <p class="text-[11px] font-bold text-slate-800 mt-1 leading-tight whitespace-normal">
                            <?php echo e($log->tahap_persetujuan == 'Pengajuan Awal' ? 'Pengajuan' : $log->tahap_persetujuan); ?>

                        </p>
                        <p class="text-[9px] font-black <?php echo e($isDone ? 'text-emerald-600' : ($isFail ? 'text-red-600' : 'text-orange-600')); ?> mt-1 italic uppercase tracking-tighter">
                            <?php echo e($logStatus); ?>

                        </p>

                        <?php if(isset($log->$timeCol) && $log->$timeCol): ?>
                            <p class="text-[8px] text-gray-400 mt-1 font-medium"><?php echo e(\Carbon\Carbon::parse($log->$timeCol)->format('d/m H:i')); ?> WIB</p>
                        <?php endif; ?>
                    </div>
                </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 h-full divide-x divide-slate-100">
        ...
        <div class="lg:col-span-2 flex flex-col min-h-0 overflow-y-auto p-4 bg-slate-50/30">
            ...
            <div class="mb-4 flex justify-between items-start">
                ...
                <span class="px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-tighter
                    <?php if($sumber == 'cuti'): ?> bg-purple-100 text-purple-600 border border-purple-200
                    <?php elseif($sumber == 'lembur'): ?> bg-cyan-100 text-cyan-600 border border-cyan-200
                    <?php else: ?> bg-orange-100 text-orange-600 border border-orange-200 <?php endif; ?>">
                    <?php echo e(strtoupper($sumber)); ?>
                ...
                <div>
                    ...
                    <p class="text-sm text-indigo-600 font-medium italic">
                        ...
                        <?php elseif($sumber == 'lembur'): ?>
                            Lembur Kerja
                        <?php elseif($sumber == 'pensiun'): ?>
                            Pensiun (<?php echo e($data->jenis_pengajuan ?? 'Normal'); ?>)
                        <?php elseif($sumber == 'pangkat'): ?>
                            Kenaikan Pangkat/Gaji/Tunjangan
                        <?php else: ?>
                            <?php echo e(ucfirst($sumber)); ?>
            ...
            <div class="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm relative">
                    ...
                    <div class="ml-4 bg-orange-50/50 p-6 rounded-2xl border border-orange-100 space-y-5">
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div>
                                ...
                                <span class="text-sm font-bold text-slate-800"><?php echo e($data->masa_kerja ?? '-'); ?> Tahun</span>
                            </div>
                            <div>
                                <span class="block text-[10px] font-bold text-orange-400 uppercase tracking-widest mb-1">TMT Pegawai</span>
                                <span class="text-sm font-semibold text-slate-700"><?php echo e(\Carbon\Carbon::parse($data->tmt_pegawai)->translatedFormat('d F Y')); ?></span>
                            </div>
                            <div>
                                <span class="block text-[10px] font-bold text-orange-400 uppercase tracking-widest mb-1">TMT Pensiun</span>
                                <span class="text-sm font-bold text-orange-600"><?php echo e(\Carbon\Carbon::parse($data->tmt_pensiun)->translatedFormat('d F Y')); ?></span>
                            </div>
                        </div>
                    </div>
                ...
                <?php else: ?>
                    ...
                    <div class="ml-4 bg-emerald-50/50 p-6 rounded-2xl border border-emerald-100">
                        ...
                        <p class="text-sm text-slate-700 leading-relaxed italic">"<?php echo e($data->uraian_perubahan ?? 'Detail perubahan sedang diproses'); ?>"</p>
                    </div>
                <?php endif; ?>
        ...
        <div class="lg:col-span-1 flex flex-col bg-slate-50 overflow-hidden border-l border-slate-100 h-full">
            ...
            <div class="flex-1 overflow-y-auto custom-scroll-container p-3 pb-5">
                ...
                <form id="formApproval" action="<?php echo e(route('direktur.approval.update', [$id_log, $sumber])); ?>" method="POST" class="flex flex-col">
                    <?php echo csrf_field(); ?>
                    ...
                    <?php else: ?>
                        <textarea name="catatan" rows="5"
                            ...
                            <?php if($data->status !== 'diproses'): ?> readonly <?php endif; ?>><?php echo e($data->komentar ?? ''); ?>
...
<?php echo $__env->make('layouts.app-direktur', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\simpegbkb\resources\views/direktur/direktur_approval.blade.php ENDPATH**/ ?>