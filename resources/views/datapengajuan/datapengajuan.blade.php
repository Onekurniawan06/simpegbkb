...
@section('content')
        ...
        <div class="bg-white p-4 rounded-l-md shadow-md">
            <form action="{{ route('datapengajuan.formDataPengajuan') }}" method="GET" class="space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    {{-- Kolom Hingga Tanggal --}}
                    <div>
                        <label for="hingga_tanggal" class="block text-sm font-medium text-gray-700 mb-1">Hingga Tanggal</label>
                        <input type="date" name="hingga_tanggal" id="hingga_tanggal" value="{{ request('hingga_tanggal') }}"
                            class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2 text-sm focus:ring-blue-500 focus:border-blue-500">
                    </div>
                    ...
                    <div>
                        ...
                        <select name="type" id="type"
                            ...
                            {{-- Sesuaikan value dengan type yang Anda gunakan di Controller --}}
                            <option value="Cuti" {{ request('type') == 'Cuti' ? 'selected' : '' }}>Pengajuan Cuti</option>
                            <option value="Lembur" {{ request('type') == 'Lembur' ? 'selected' : '' }}>Pengajuan Lembur</option>
                            <option value="Pensiun" {{ request('type') == 'Pensiun' ? 'selected' : '' }}>Pengajuan Pensiun</option>
                            <option value="PangkatGajiTunjangan" {{ request('type') == 'PangkatGajiTunjangan' ? 'selected' : '' }}>Kenaikan Pangkat, Gaji dan Tunjangan</option>
                        </select>
                    </div>

                    {{-- Kolom Status Pengajuan (BARU DITAMBAHKAN) --}}
                    <div>
                        ...
                        <select name="status_pengajuan_filter" id="status_pengajuan_filter"
                            ...
                            {{-- Sesuaikan value dengan status yang Anda simpan di database/enum --}}
                            <option value="diproses" {{ request('status_pengajuan_filter') == 'diproses' ? 'selected' : '' }}>Diproses</option>
                            <option value="disetujui" {{ request('status_pengajuan_filter') == 'disetujui' ? 'selected' : '' }}>Disetujui</option>
                            <option value="ditolak" {{ request('status_pengajuan_filter') == 'ditolak' ? 'selected' : '' }}>Ditolak</option>
                        </select>
                    </div>
                </div>
    ...
    <div class="bg-white shadow-sm rounded-tl-md overflow-hidden border border-gray-100 mt-2 h-full flex flex-col">
        {{-- Pengecekan Data: Jika submissions KOSONG --}}
        ...
        @else
                ...
                @foreach ($submissions as $submission)
                    <div class="group relative flex items-center bg-white p-4 rounded-md border border-gray-100 shadow-sm hover:shadow-md hover:border-blue-200 transition-all duration-300">
                        ...
                        <div class="absolute left-0 top-4 bottom-4 w-1 rounded-r-full
                            {{ $submission['type'] === 'Cuti' ? 'bg-indigo-500' :
                        ...
                        {{-- Konten Kiri: Info Utama & Semua Detail --}}
                        <div class="pl-3 w-[380px] shrink-0">
                            <div class="flex items-center gap-3 mb-2">
                                <h4 class="text-sm font-semibold">
                                    {{ $submission['type'] == 'PangkatGajiTunjangan' ? ($submission['jenis_pengajuan'] ?? 'Kenaikan') : $submission['type'] }}
                                </h4>
                                <span class="text-[11px] font-bold px-2 py-0.5 rounded bg-slate-100 text-slate-500 uppercase tracking-tighter">
                                    {{ isset($submission['created_at']) ? \Carbon\Carbon::parse($submission['created_at'])->format('d M Y') : '-' }}
                                </span>
                            </div>

                            {{-- Detail Spesifik (Data UTUH sesuai kode awal kamu) --}}
                            <div class="text-[12px] text-slate-500 space-y-0.5">
                                @if ($submission['type'] === 'Cuti')
                                    <div class="flex items-center gap-2">
                                        <span class="w-1 h-1 rounded-full bg-slate-300"></span>
                                        <span>Jenis Cuti : <b class="text-slate-700">{{ $submission['jenis_cuti'] ?? 'Tidak ada keterangan' }}</b></span>
                                    </div>
                                    <div class="flex items-center gap-2">
                                        <span class="w-1 h-1 rounded-full bg-slate-300"></span>
                                        <span>Tanggal : <b class="text-slate-700">{{ isset($submission['tanggal_mulai']) ? \Carbon\Carbon::parse($submission['tanggal_mulai'])->format('d F Y') : '-' }} - {{ isset($submission['tanggal_selesai']) ? \Carbon\Carbon::parse($submission['tanggal_selesai'])->format('d F Y') : '-' }}</b></span>
                                    </div>
                                    <div class="flex items-center gap-2 italic">
                                        <span class="w-1 h-1 rounded-full bg-slate-300"></span>
                                        <span>Alasan : <b class="text-slate-700">"{{ $submission['keterangan'] ?? 'Tidak ada keterangan' }}"</b></span>
                                    </div>

                                @elseif ($submission['type'] === 'Lembur')
                                    <div class="flex items-center gap-2">
                                        <span class="w-1 h-1 rounded-full bg-slate-300"></span>
                                        <span>Tanggal Lembur : <b class="text-slate-700">{{ isset($submission['tanggal_lembur']) ? \Carbon\Carbon::parse($submission['tanggal_lembur'])->format('d F Y') : '-' }}</b></span>
                                    </div>
                                    <div class="flex items-center gap-2">
                                        <span class="w-1 h-1 rounded-full bg-slate-300"></span>
                                        <span>Jam : <b class="text-slate-700">{{ $submission['jam_mulai'] ?? '-' }} - {{ $submission['jam_selesai'] ?? '-' }}</b></span>
                                    </div>
                                    <div class="flex items-center gap-2 italic">
                                        <span class="w-1 h-1 rounded-full bg-slate-300"></span>
                                        <span>Tugas : <b class="text-slate-700">"{{ $submission['uraian_tugas'] ?? 'Tidak ada keterangan' }}"</b></span>
                                    </div>

                                @elseif (($submission['type'] ?? '') === 'Pensiun')
                                    <div class="flex items-center gap-2">
                                        <span class="w-1 h-1 rounded-full bg-slate-300"></span>
                                        <span>TMT Pensiun : <b class="text-slate-700">{{ isset($submission['tmt_pensiun']) ? \Carbon\Carbon::parse($submission['tmt_pensiun'])->format('d F Y') : 'N/A' }}</b></span>
                                    </div>
                                    <div class="flex items-center gap-2">
                                        <span class="w-1 h-1 rounded-full bg-slate-300"></span>
                                        <span>TMT Pegawai : <b class="text-slate-700">{{ isset($submission['tmt_pegawai']) ? \Carbon\Carbon::parse($submission['tmt_pegawai'])->format('d F Y') : 'N/A' }}</b></span>
                                    </div>
                                    <div class="flex items-center gap-2">
                                        <span class="w-1 h-1 rounded-full bg-slate-300"></span>
                                        <span>Masa Kerja : <b class="text-slate-700">{{ $submission['masa_kerja'] ?? 'N/A' }}</b></span>
                                    </div>
                        ...
                        {{-- Konten Tengah: Sekarang dia akan menempel setelah jarak 380px dari kiri --}}
                        <div class="px-8 hidden md:block border-l border-gray-100">
                            <div class="flex flex-col items-center">
                                ...
                                <div class="inline-flex items-center px-3 py-1 rounded-lg border {{ $submission['blade_class'] ?? 'bg-amber-50 text-amber-700 border-amber-200' }} shadow-sm">
                                    ...
                                    <span class="text-[10px] font-extrabold uppercase whitespace-nowrap">
                                        {{ $submission['blade_status_text'] }} - {{ $submission['blade_stage'] }}
                                    </span>
                                </div>
                            </div>
                        </div>

                        {{-- Spacer: Ini yang akan mengambil sisa ruang agar tombol tetap di kanan --}}
            ...
            <div class="mt-0 px-4 py-3 bg-slate-50/50 border-t border-slate-100 flex justify-between items-center gap-4 rounded-b-xl">
                ...
                {{-- Hilangkan teks "Showing" dengan class khusus --}}
