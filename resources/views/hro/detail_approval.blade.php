        ...
        <div class="relative flex items-start">
            ...
            @foreach($historiLog as $index => $log)
                ...
                <div class="flex flex-col items-center flex-1 relative">
                    ...
                    @if(!$loop->last)
                        {{-- Garis antar titik di database (Normal) --}}
                        <div class="absolute top-5 left-1/2 w-full h-[2.5px] z-0 {{ $lineColor }}"></div>
                    @else
                        {{-- LOGIKA BARU: Garis di titik terakhir database --}}
                        {{-- Garis HANYA muncul jika pengajuan BELUM Selesai dan BELUM Ditolak --}}
                        @if($log->tahap_persetujuan !== 'Selesai' && !$isFailFinal)
                            <div class="absolute top-5 left-1/2 w-full h-[2.5px] z-0 {{ $isCurr ? 'bg-orange-500' : 'bg-gray-200' }}"></div>
                        @endif
                    @endif
            ...
            @if(!$isHRODone && !$isFailFinal && $lastLog->tahap_persetujuan !== 'Selesai')
                <div class="flex flex-col items-center flex-1 relative">
