...
@section('content')
    ...
    <div class="bg-white rounded-l-md shadow-md border border-gray-200 overflow-hidden">
        ...
        <h2 class="text-xl font-bold tracking-tight" style="color: white !important;">{{ __('Ubah Kata Sandi') }}</h2>
                ...
                <div class="bg-gray-50 rounded-xl p-4 border border-gray-200 mb-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
                        ...
                        <div class="md:col-span-1">
                            ...
                            <input type="text" value="{{ $user->nomor_urut_pegawai }}" class="w-full px-4 py-2 bg-gray-100 border border-gray-200 text-gray-500 text-sm rounded-lg cursor-not-allowed focus:outline-none" readonly>
                        </div>
                        <!-- Field 2: Nama Lengkap (span 3 dari 5 kolom) -->
                        <div class="md:col-span-2">
                            <label class="block text-sm font-medium text-gray-700 mb-1">Nama Lengkap</label>
                            <input type="text" value="{{ $user->name }}" class="w-full px-4 py-2 bg-gray-100 border border-gray-200 text-gray-500 text-sm rounded-lg cursor-not-allowed focus:outline-none" readonly>
                        </div>
                        <!-- Field 3: Alamat Email (span 1 dari 5 kolom) -->
                        <div class="md:col-span-2">
                            <label class="block text-sm font-medium text-gray-700 mb-1">Alamat Email</label>
                            <input type="email" value="{{ $user->email }}" class="w-full px-4 py-2 bg-gray-100 border border-gray-200 text-gray-500 text-sm rounded-lg cursor-not-allowed focus:outline-none" readonly>
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div class="space-y-2">
                        ...
                        @error('current_password')
                            <p class="mt-1 text-[11px] text-red-500 font-medium">{{ $message }}</p>
                        @enderror
                    </div>
