...
@section('content')
{{-- @php
    // Tampilkan semua data session yang diterima di halaman ini
    dump(session()->all());
...
@endphp --}}
    ...
    <form action="{{ route('pegawai.updateCutiizin') }}" method="POST" enctype="multipart/form-data" id="leaveForm">
        @csrf
        {{-- @method('PUT') --}}
        ...
        <div class="bg-white rounded-lg shadow-lg max-w-full mx-auto">
            ...
            <div style="background-image: linear-gradient(rgba(255, 255, 255, 0.7), rgba(255, 255, 255, 0.1)), url('{{ asset('images/vecteezylight.jpg') }}')" class="bg-cover bg-bottom p-2 rounded-t-lg relative">
                <img src="{{ asset('images/vacation.png') }}" alt="Overtime" class="absolute right-0 top-0 h-48">
            ...
            <h3 class="text-lg font-semibold text-blue-700 mb-4"># Section 1: Pengajuan Cuti dan Izin</h3>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        <label for="jenis_cuti" class="block text-sm font-medium text-gray-700">Jenis Cuti dan Izin</label>
                    ...
                    <div id="sub_jenis_cuti_container" style="display: none;">
                        <label for="sub_jenis_cuti" class="block text-sm font-medium text-gray-700">Sub Jenis Cuti Penting</label>
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                        <div class="flex flex-col gap-1">
                            <label for="jatah_periode_hari" class="block text-sm font-medium text-gray-700">Cuti/ Izin yang didapat</label>
                        ...
                        <div class="flex flex-col gap-1">
                            ...
                            <input
                                id="tanggal_mulai"
                                type="date"
                                {{-- Penambahan atribut min menggunakan fungsi server-side --}}
                                min="{{ now()->format('Y-m-d') }}"
                                class="form-control @error('tanggal_mulai') is-invalid @enderror w-full bg-white text-slate-700 text-sm border border-slate-200 rounded-md px-3 py-2 focus:border-blue-500 outline-none shadow-sm"
                                name="tanggal_mulai"
                                required
                            >
                            @error('tanggal_mulai')
                        ...
                        <div class="flex flex-col gap-1">
                            <label for="tanggal_selesai" class="block text-sm font-medium text-gray-700">Tanggal Masuk Kerja</label>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="flex flex-col gap-1">
                            <label for="jumlah_cuti" class="block text-sm font-medium text-gray-700">Jumlah Cuti yang diambil(Hari)</label>
                        ...
                        <div class="flex flex-col gap-1">
                            <label for="sisa_cuti" class="block text-sm font-medium text-gray-700">Sisa Cuti (Hari)</label>
                    ...
                    @php
                        // Pastikan variabel-variabel ini tersedia dari Controller Anda
                        $jenisCutiJson = $jenisCuti->toJson();
                        $jatahCutiTahunanMaksimal = $jenisCuti->where('nama_cuti', 'Cuti Tahunan')->first()->durasi_hari ?? 0;
                        // $subJenisCuti juga harus tersedia untuk struktur HTML div container
                    @endphp
                </div>
        ...
        <div class="bg-white rounded-lg shadow-lg max-w-full mx-auto">
            <div class="mb-4 p-4 shadow-sm" style="background-color: white; border-radius: 12px; border: 1px solid #e5e7eb;">
                <h3 class="text-lg font-semibold text-blue-700 mb-4"># Section 3: Keterangan Cuti/ Izin</h3>
                <div class="upload-group">
                    <label for="keterangan" class="block text-sm font-medium mb-2 text-gray-700">Keterangan Cuti & izin:</label>
                    <textarea name="keterangan" id="keterangan" rows="3" placeholder="Masukkan alasan/keterangan cuti anda..."
                    class="w-full bg-white text-slate-700 text-sm border border-slate-200 rounded-md px-3 py-2 focus:border-blue-500 outline-none shadow-sm"></textarea>
                </div>
        ...
        <div id="successModal" class="{{ Session::has('success') ? 'show-on-load' : '' }} fixed inset-0 bg-black/50 items-center justify-center hidden z-50 backdrop-blur-sm">
            <div class="bg-gray-50 p-8 rounded-xl shadow-2xl w-full max-w-lg mx-4">
                ...
                <div class="mt-6 flex justify-center space-x-4">
                    ...
                    {{-- <a href="#" id="downloadPdfButton" class="flex items-center px-4 py-2 bg-blue-100 text-sm text-blue-600 rounded-lg hover:bg-blue-200 transition-colors">
                        <!-- Icon Download -->
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="www.w3.org"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                        Download Bukti Pengajuan
                    </a> --}}
                    <!-- 2. Tombol Lihat Pengajuan (Menggunakan route yang Anda sebutkan sebelumnya) -->
                    <a href="{{ route('pegawai.formDataPengajuan', ['type' => 'cuti']) }}" class="px-4 py-2 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700 transition-colors">
                        Lihat Pengajuan
                    </a>
                    <!-- 3. Tombol Tutup (Dengan ID untuk JS Listener) -->
                    {{-- <button id="closeSuccessModalButton" type="button" class="px-4 py-2 bg-gray-300 text-gray-800 rounded-lg hover:bg-gray-400 transition-colors">
                        Tutup
                    </button> --}}
                </div>
            </div>
...
{{-- Script Perhitungan Sisa Cuti --}}
{{-- Pastikan variabel PHP ini tersedia dari Controller Anda tepat di atas script --}}
@php
    $jenisCutiJson = $jenisCuti->toJson();
    $jatahCutiTahunanMaksimal = $jenisCuti->where('nama_cuti', 'Cuti Tahunan')->first()->durasi_hari ?? 0;
@endphp

<!-- Memanggil file JS eksternal (menggunakan Vite untuk Laravel modern) -->
