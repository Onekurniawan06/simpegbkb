...
@section('content')
    ...
    <div class="bg-white rounded-lg shadow-lg max-w-full mx-auto">
        ...
        <div style="background-image: linear-gradient(rgba(255, 255, 255, 0.7), rgba(255, 255, 255, 0.1)), url('{{ asset('images/vecteezylight.jpg') }}')" class="bg-cover bg-bottom p-2 rounded-t-lg relative">
            ...
            <div class="flex items-center mt-3 ml-4 mb-4">
                <!-- Kontainer Foto Profil Interaktif dan Form Upload -->
                <div class="relative group">
                    {{-- Form Unggah Foto --}}
                    <form action="{{ route('pegawai.profile.updatePhoto') }}" method="POST" enctype="multipart/form-data" id="photoForm">
                        @csrf
                        @method('PATCH')
        ...
        <div class="mb-4 p-4 shadow-sm">
            <h3 class="text-lg font-semibold text-blue-700"># Section 1: Detail Pribadi</h3>
            ...
            <div x-data="{ activeTab: 'pribadi' }" class="">
                ...
                <form action="{{ route('pegawai.profile.updateProfile') }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="flex gap-4">
                                <div class="flex-1">
                                    ...
                                    value="{{ old('tempat_lahir', $detailPribadi->tempat_lahir ?? '') }}" class="w-full bg-transparent placeholder:text-slate-400 text-slate-700 text-sm border border-slate-200 rounded-md px-3 py-2 mt-1 transition duration-300 ease focus:outline-none focus:border-slate-400 hover:border-slate-300 shadow-sm focus:shadow read-only:bg-gray-50 @error('tempat_lahir') border-red-500 @enderror" placeholder="Tempat Lahir"
                                ...
                                <div class="flex-1">
                                    ...
                                    <div class="relative mt-1">
                                            <input type="text" name="tanggal_lahir" id="tanggal_lahir"
                                                value="{{ old('tanggal_lahir', isset($detailPribadi->tanggal_lahir) ? \Carbon\Carbon::parse($detailPribadi->tanggal_lahir)->format('Y-m-d') : '') }}"
                            ...
                            <div class="flex-1">
                                ...
                                <select name="pendidikan_terakhir" id="pendidikan_terakhir" class="w-full bg-white text-slate-700 text-sm border border-slate-200 rounded-md px-3 py-2 mt-1 transition duration-300 ease focus:outline-none focus:border-slate-400 hover:border-slate-300 shadow-sm focus:shadow" >
                                    ...
                                    @php
                                        $pendidikan_terakhir = ['Sekolah Dasar (SD)', 'Sekolah Menengah Pertama (SMP)', 'Sekolah Menengah Atas/ Kejuruan (SMA/SMK)', 'Diploma III (D3)', 'Sarjana (S1)', 'Magister (S2)', 'Doktor (S3)'];
                                    @endphp
                                    @foreach ($pendidikan_terakhir as $pendidikan)
                                        <option value="{{ $pendidikan }}" {{ (old('pendidikan_terakhir', Auth::user()->detailPribadi->pendidikan_terakhir ?? '') == $pendidikan) ? 'selected' : '' }}>
                                            {{ $pendidikan }}
                                        </option>
                                    @endforeach
                                </select>
                    ...
                    <div x-show="activeTab === 'dokumen'" class="space-y-6" x-cloak>
    ...
    <div class="bg-white rounded-lg shadow-lg max-w-full mx-auto">
        <div class="mb-4 p-4 shadow-sm">
            <h3 class="text-lg font-semibold mb-4 text-blue-700"># Section 2: Data Keluarga</h3>
            <form action="{{ route('pegawai.profile.updateKeluarga') }}" method="POST">
                @csrf
                @method('PUT')
                {{-- resources/views/pegawai/profile.blade.php --}}
    ...
    <div class="bg-white rounded-lg shadow-lg max-w-full mx-auto">
        <div class="mb-4 p-4 shadow-sm">
            <h3 class="text-lg font-semibold mb-4 text-blue-700"># Section 3: Status Kepegawaian</h3>
            <form action="{{ route('pegawai.profile.updatePekerjaan') }}" method="POST">
                @csrf
                @method('PUT')
