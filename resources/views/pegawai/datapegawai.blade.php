    ...
    <div class="p-6 bg-gradient-to-br from-[#001A4E] via-[#002b7a] to-[#001A4E] flex-none relative overflow-hidden">
        ...
        <div class="relative z-10">
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
                <div class="flex items-center gap-4">
                    ...
                    <h2 class="text-3xl font-black text-white tracking-tighter">
                        {{ $pegawaiDivisi->total() }} <span class="text-blue-100 font-light text-xl ml-2">Pegawai</span>
                    </h2>
                </div>

                <!-- SEARCH & FILTER BOX -->
                <form action="{{ route('pegawai.data') }}" method="GET" class="flex items-center gap-3">
                    @php
                        ...
                        <div class="relative group">
                            <select name="divisi" onchange="this.form.submit()"
                                ...
                                @foreach($allDivisi as $d)
                                    <option value="{{ $d->id_divisi }}" {{ request('divisi') == $d->id_divisi ? 'selected' : '' }} class="text-slate-800">
                                        {{ $d->nama_divisi }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    @if(request('search') || request('divisi'))
                        <a href="{{ route('pegawai.data') }}" class="px-4 py-2.5 bg-red-500 text-white rounded-2xl text-xs font-black hover:bg-red-600 transition-all flex items-center gap-2 shadow-lg uppercase tracking-wider">
                            RESET
                        </a>
                    @endif
                </form>
            ...
            <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
                @foreach($statusConfigs as $label => $style)
                <div class="bg-white/10 border border-white/10 backdrop-blur-xl p-4 rounded-2xl hover:bg-white/20 transition-all duration-300 group shadow-lg">
                    <div class="flex items-center gap-2 mb-2.5">
                        <span class="w-2 h-2 rounded-full {{ $style['dot'] }} shadow-[0_0_10px_currentColor]"></span>
                        <span class="text-[10px] font-black text-blue-100 uppercase tracking-widest leading-none">{{ $label }}</span>
                    </div>
                    <div class="flex items-baseline gap-2">
                        <span class="text-2xl font-black text-white tracking-tight">{{ $stats[$label] ?? 0 }}</span>
                    ...
                    <div class="w-full h-1 bg-white/10 rounded-full mt-3 overflow-hidden">
                        <div class="h-full {{ $style['dot'] }} opacity-70" style="width: {{ $pegawaiDivisi->total() > 0 ? (($stats[$label] ?? 0) / $pegawaiDivisi->total()) * 100 : 0 }}%"></div>
                    </div>
                </div>
                @endforeach
            </div>
        ...
        <div class="flex-1 overflow-y-auto custom-scroll-container">
            ...
            <table class="w-full text-left border-separate border-spacing-0 table-fixed">
                ...
                <tbody class="divide-y divide-slate-100 bg-white">
                    ...
                    <tr class="hover:bg-blue-50/50 transition-all duration-200 group"> {{-- X-SHOW DIHAPUS --}}
                        <td class="py-3 px-6">
                            <span class="font-mono text-[13px] font-black {{ $p->level_id == 3 ? 'text-blue-700' : 'text-slate-500' }} tracking-tight">
                                #{{ $p->nomor_urut_pegawai }}
                            </span>
                        </td>
                        <td class="py-3 px-6">
                            <div class="flex flex-col">
                                <span class="font-bold text-slate-700 text-[13px] tracking-tight group-hover:text-blue-700 uppercase leading-tight">{{ $p->nama }}</span>
                                @if($p->level_id == 3)
                                ...
                                @else
                                    <span class="mt-1 text-[10px] text-slate-400 font-medium italic leading-none">{{ $p->nama_divisi }}</span>
                                @endif
                            </div>
                        </td>
                        <td class="py-3 px-6">
                            <div class="text-[12px] text-slate-600 font-bold leading-snug line-clamp-1" title="{{ $p->jabatan }}">{{ $p->jabatan }}</div>
                            <div class="flex items-center gap-2 mt-1">
                                @if($p->grade)
                                    <span class="px-1.5 py-0.5 bg-blue-50 text-blue-600 text-[10px] font-bold rounded border border-blue-100">Grade {{ $p->grade }}</span>
                                    <span class="text-[10px] text-slate-400 font-medium italic opacity-80">{{ $p->pangkat }}</span>
                                @else
                        ...
                        <td class="py-3 px-6 text-center">
                            @php $currStyle = $statusStyles[$p->status_pegawai] ?? ['bg' => 'bg-slate-50', 'text' => 'text-slate-400', 'border' => 'border-slate-100']; @endphp
                            <span class="{{ $currStyle['bg'] }} {{ $currStyle['text'] }} text-[10px] px-3 py-1.5 rounded-full font-black uppercase border {{ $currStyle['border'] }} shadow-sm">
                                {{ $p->status_pegawai ?? 'Aktif' }}
                            </span>
                        </td>
                        <td class="py-3 px-6 text-center">
                            <a href="{{ route('pegawai.detail', $p->nomor_urut_pegawai) }}" class="inline-flex items-center gap-1.5 bg-white text-slate-600 border border-slate-200 px-3 py-1.5 rounded-lg text-[10px] font-bold hover:bg-[#001A4E] hover:text-white transition-all shadow-sm">
                                Lihat Detail
                            </a>
                        </td>
                    </tr>
    ...
    <div class="p-4 px-8 border-t border-slate-100 bg-white flex-none">
        <div class="flex justify-between items-center">
            ...
            <span class="text-[10px] text-slate-400 font-bold uppercase tracking-widest">
                Menampilkan {{ $pegawaiDivisi->firstItem() }} - {{ $pegawaiDivisi->lastItem() }} dari {{ $pegawaiDivisi->total() }} Pegawai
            </span>
