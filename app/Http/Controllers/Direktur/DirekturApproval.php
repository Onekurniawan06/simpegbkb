    ...
    public function index(Request $request)
{
    ...
    // 1. Ambil data role (TETAP SEPERTI KODEMU)
    ...
    // 2. NORMALISASI (TETAP SEPERTI KODEMU)
    ...
    // 3. DEFINISI TABEL (SUDAH DITAMBAHKAN PENSIUN)
    $tables = [
    ...
    public function updateStatus(Request $request, $sumber, $id_log)
{
    try {
        ...
        // Normalisasi $sumber agar tidak sensitif huruf besar/kecil
        ...
        if ($status === 'disetujui') {
            ...
            // C. Mapping ID Transaksi (🔄 PERBAIKAN: Agar ID_KENAIKAN Tidak NULL lagi)
            if ($sumber === 'cuti') {
                $dataInsert['id_cuti'] = $oldLog->id_cuti;
            } elseif ($sumber === 'pensiun') {
                $dataInsert['id_pensiun'] = $oldLog->id_pensiun;
            }
            // Gunakan in_array agar aman untuk slug pendek maupun panjang
...
public function lihatDokumen($id)
{
    ...
    // 2. Tentukan Path (Pastikan path ini sinkron dengan cara simpan di Controller Update)
    $path = storage_path('app/private/' . $file->path_file_server);

    // 🛡️ Safety Net: Jika folder 'private' sudah masuk di kolom database,
    // agar tidak double path, kita cek fisiknya secara fleksibel
    if (!file_exists($path)) {
    ...
    // 3. ✨ KUNCI PERBAIKAN: Deteksi Mime-Type Otomatis (Bisa PDF, JPG, PNG)
