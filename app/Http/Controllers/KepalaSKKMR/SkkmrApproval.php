...
class SkkmrApproval extends Controller
{
    ...
    public function detailApproval($sumber, $id_log)
    {
        ...
        // Mendaftarkan kembali lembur di sini
        ...
        } elseif ($sumber === 'pangkatgajitunjangan' && isset($data->id_kenaikan)) {
        ...
        // 2. Paksa $sumber menjadi huruf kecil saat dicocokkan
    ...
    public function updateStatus(Request $request, $sumber, $id_log)
    {
        ...
        if ($request->status === 'disetujui') {
            ...
            if ($sumber === 'pensiun') {
                $insertBase['id_pensiun'] = $logLama->id_pensiun;
            } elseif ($sumber === 'lembur') {
                $insertBase['id_lembur'] = $logLama->id_lembur;
            } else {
                $insertBase['id_kenaikan'] = $logLama->id_kenaikan ?? null;
            }
    ...
    {
        // 1. Cari dulu di tabel pensiun
        $file = \DB::table('file_persyaratanpensiun')->where('id', $id)->first();

        // 2. Jika tidak ada di pensiun, cari di tabel pangkat
        if (!$file) {
            $file = \DB::table('file_persyaratanpangkatgajitunjangan')->where('id', $id)->first();
        }

        // 🛡️ Cek apakah data di DB ada setelah mencari di kedua tabel
        if (!$file) {
        ...
        // 🛡️ Cek apakah file fisiknya benar-benar ada di folder
        if (!file_exists($path)) {
            abort(404, 'File fisik tidak ditemukan di folder storage. Silakan unggah ulang dokumen.');
        }
